...
inoremap <nowait> <silent> <expr> <S-Tab> coc#pum#visible() ? coc#pum#next(1) : "\<S-Tab>"
snoremap <silent>  c
nnoremap  h
nnoremap <NL> j
nnoremap  k
...
inoremap <nowait> <silent> <expr> 	 coc#pum#visible() ? coc#pum#confirm() : "\	"
...
set incsearch
set laststatus=2
set runtimepath=~/.vim,~/.vim/plugged/nerdtree,~/.vim/plugged/ale,~/.vim/plugged/gruvbox,~/.vim/plugged/coc.nvim,~/.vim/plugged/auto-pairs,~/.vim/plugged/vim-expand-region,~/.vim/plugged/traces.vim,~/.vim/plugged/jupytext.vim,/var/lib/vim/addons,/etc/vim,/usr/share/vim/vimfiles,/usr/share/vim/vim90,/usr/share/vim/vimfiles/after,/etc/vim/after,/var/lib/vim/addons/after,~/.vim/after
set shiftwidth=2
set showcmd
set showmatch
set smartindent
...
silent only
silent tabonly
cd ~/University/Term6/PR/GeneticAlgorithm
...
exe 'vert 2resize ' . ((&columns * 159 + 135) / 271)
exe '3resize ' . ((&lines * 33 + 34) / 68)
exe 'vert 3resize ' . ((&columns * 79 + 135) / 271)
exe '4resize ' . ((&lines * 33 + 34) / 68)
exe 'vert 4resize ' . ((&columns * 79 + 135) / 271)
exe '5resize ' . ((&lines * 17 + 34) / 68)
exe 'vert 5resize ' . ((&columns * 79 + 135) / 271)
exe '6resize ' . ((&lines * 17 + 34) / 68)
exe 'vert 6resize ' . ((&columns * 79 + 135) / 271)
exe '7resize ' . ((&lines * 30 + 34) / 68)
exe 'vert 7resize ' . ((&columns * 79 + 135) / 271)
argglobal
enew
file NERD_tree_1
balt common.h
...
197
normal! zo
let s:l = 101 - ((100 * winheight(0) + 16) / 32)
...
setlocal nowrap
setlocal wrapmargin=0
let s:l = 280 - ((3 * winheight(0) + 16) / 33)
if s:l < 1 | let s:l = 1 | endif
keepjumps exe s:l
normal! zt
keepjumps 280
...
wincmd w
argglobal
if bufexists(fnamemodify("util.h", ":p")) | buffer util.h | else | edit util.h | endif
balt common.h
let s:cpo_save=&cpo
set cpo&vim
inoremap <buffer> <silent> <M-n> :call AutoPairsJump()a
...
setlocal nowrap
setlocal wrapmargin=0
let s:l = 26 - ((15 * winheight(0) + 8) / 17)
if s:l < 1 | let s:l = 1 | endif
keepjumps exe s:l
normal! zt
keepjumps 26
...
wincmd w
argglobal
if bufexists(fnamemodify("common.h", ":p")) | buffer common.h | else | edit common.h | endif
balt seq.cpp
let s:cpo_save=&cpo
set cpo&vim
inoremap <buffer> <silent> <M-n> :call AutoPairsJump()a
...
wincmd w
argglobal
if bufexists(fnamemodify("seq.cpp", ":p")) | buffer seq.cpp | else | edit seq.cpp | endif
balt seq.cpp
let s:cpo_save=&cpo
set cpo&vim
inoremap <buffer> <silent> <M-n> :call AutoPairsJump()a
